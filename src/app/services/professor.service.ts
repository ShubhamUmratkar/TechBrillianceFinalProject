...
export class ProfessorService 
{
  ...
  getDriveCourseList() : Observable<any>
  {
  ...
  getCourseListNames() : Observable<any>
  {
    ...
    getProfileImage(userEmail: string): Observable<Blob> {
      return this._http.get(`${NAV_URL}/getprofile/${userEmail}`, { responseType: 'blob' });
    }
  
    // Method to upload the profile image
    uploadProfileImage(userEmail: string, formData: FormData): Observable<any> {
      return this._http.post(`${NAV_URL}/uploadimage/${userEmail}`, formData, { responseType: 'text' });
    }
}
